<template>  
  <section class="modlude">
    ...
    <el-table :data="formData" style="width: 100%" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="" label="接听权限">
        <template slot-scope="scope">
          {{scope.row.isCall==1?"有":'无'}}
        </template>
      </el-table-column>
    ...
    <el-dialog title="新增" :visible.sync="addDialog" :close-on-click-modal="false">
      <div class="cont_box_right">
        <el-form label-position="right" :rules="rules" label-width="80px" :model="formPush" ref='addList'> 
          ...
          <el-form-item label="接听权限" size="small">
            <el-checkbox v-model="isCall" label=" "></el-checkbox>
            <el-tooltip content="作为APP一键呼叫物业接听账户,目前暂只支持一人接听：勾选此框作为物业接听账户" placement="top">
              <el-button size="mini">提示</el-button>
            </el-tooltip>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods:{
        ...
        if (valid) {
          ...
          pmtypePhone(params).then((ress)=>{
            ...
            if(ress.data.code == 200){
              this.formPush.isCall = this.isCall?'1':'0'
        ...
        if (valid) {
          ...
          pmtypePhone(params).then((ress)=>{
            ...
            if(ress.data.code == 200){
              ...
              this.formUpdate.isCall = this.isCall?'1':'0'
    ...
    updateShowBox(item){//修改东西弹窗
      ...
      this.isCall = this.formUpdate.isCall?true:false
