...
import {getgenreid,getlishopinfo,deletePinglunHouell,pinglunHouell,xqSelectList,orgTree } from '../../url/api';
export default {
  data(){
    return{
      checkStrictly:false,
  ...
  methods:{
    getlist(){
      ...
      pinglunHouell(this.formSearch).then((res)=>{
        console.log(res)
    ...
    addList(addList){//添加树状图node节点
      this.$refs[addList].validate((valid) => {
        if (valid) {
          addpinglunHouell(this.formPush).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.getlist()
              this.$message({
    ...
    updateList(formName){//修改
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          updatepinglunHouell(params).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.$message({
                message: '修改成功',
    ...
    deleInfor(ids){//删除
      ...
      this.$confirm('确认删除吗？')
      .then(_ => {
        deletePinglunHouell(arrId).then((res)=>{
          console.log(res)
          if(res.data.code == 200){
            this.$message('删除成功');
            this.getlist()
