  ...
  <section class="modlude">
    <el-form :inline="true" :model="formSearch" class="form_inline" label-width="90px">
      <el-form-item label="评论内容" size="small">
        <el-input v-model="formSearch.commentInfo" placeholder="请输入评论内容"></el-input>
      </el-form-item>
      <el-form-item label="评论人昵称" size="small">
        <el-input v-model="formSearch.fromSysName" placeholder="请输入评论人昵称"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="formData" style="width: 100%" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="fromSysName" label="评论人昵称"></el-table-column>
      <el-table-column prop="fromSysJyh" label="评论人家园号"></el-table-column>
      <el-table-column prop="toSysJyh" label="被评论人家园号"></el-table-column>
      <el-table-column prop="toSysName	" label="被评论人昵称"></el-table-column>
...
import {getgenreid,getlishopinfo,deletepinglunCraftsman,pinglunCraftsman,xqSelectList,orgTree } from '../../url/api';
export default {
  data(){
    return{
      checkStrictly:false,
  ...
  methods:{
    getlist(){
      this.formSearch.id=this.$route.query.fkId
      pinglunCraftsman(this.formSearch).then((res)=>{
        console.log(res)
    ...
    addList(addList){//添加树状图node节点
      this.$refs[addList].validate((valid) => {
        if (valid) {
          addpinglunCraftsman(this.formPush).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.getlist()
              this.$message({
    ...
    updateList(formName){//修改
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          updatepinglunCraftsman(params).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.$message({
                message: '修改成功',
    ...
    deleInfor(ids){//删除
      ...
      this.$confirm('确认删除吗？')
      .then(_ => {
        deletepinglunCraftsman(arrId).then((res)=>{
          console.log(res)
          if(res.data.code == 200){
            this.$message('删除成功');
            this.getlist()
