<template>
  <section class="modlude">
    ...
    <el-table :data="formData" style="width: 100%" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="areaCode" label="区域编号"></el-table-column>
      <el-table-column prop="placeCount" label="区域车位数"></el-table-column>
      <el-table-column prop="areaFreeSpaceNum" label="区域剩余车位"></el-table-column>
...
import { carParkSummaryData,carParkSummaryInfo,delCar,addPropertyTypeList,updatePropertyTypeList,addReply,xqSelectList,orgTree } from '../../url/api';
export default {
  data(){
    return{
      checkStrictly:false,
  ...
  methods:{
    getlist(){
      ...
      carParkSummaryInfo(this.formSearch).then((res)=>{
        console.log(res)
        if(res.data.code == 200){
