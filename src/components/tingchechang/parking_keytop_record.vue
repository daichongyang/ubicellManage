  ...
  <section class="modlude">
    <el-form :inline="true" :model="formSearch" class="form_inline" label-width="100px">
      ...
      <el-form-item label="车牌号" size="small">
        <el-input v-model="formSearch.plateNo" placeholder="请输入车牌号"></el-input>
      </el-form-item>
      <el-form-item label="是否缴费" size="small">
        <el-select v-model="formSearch.isCharge">
    ...
    <el-table :data="formData" style="width: 100%" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="freeMoney"label="免费金额"></el-table-column>
      <el-table-column prop="duration"label="停车时长"></el-table-column>
      <el-table-column prop=""label="入场时间" width=150>>
        <template slot-scope="scope" v-if="scope.row.entryTime">
          {{$root.getDateArray(scope.row.entryTime)[9]}}
        </template>
      </el-table-column>
      ...
      <el-table-column prop=""label="是否缴费">
        <template slot-scope="scope">
          {{scope.row.status==0?'否':"是"}}
        </template>
      </el-table-column>
    </el-table>
    <paging @changePage = handleCurrentPage :get-total='total' :module-infor="moduleInfor"></paging>
    <!-- 添加 -->
  </section>  
</template>
...
import { carParkRecords,carParkRecordsFee,delCar,addPropertyTypeList,updatePropertyTypeList,addReply,xqSelectList,orgTree } from '../../url/api';
export default {
  data(){
    return{
      checkStrictly:false,
  ...
  methods:{
    getlist(){
      carParkRecords(this.formSearch).then((res)=>{
        console.log(res)
      ...
      carParkRecordsFee(this.formSearch).then((res)=>{
        console.log(res)
        if(res.data.code == 200){
          this.moduleInfor = {
