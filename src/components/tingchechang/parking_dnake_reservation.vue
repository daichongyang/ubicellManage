  ...
  <section class="modlude">
    <el-form :inline="true" :model="formSearch" class="form_inline" label-width="100px">
      ...
      <el-form-item label="车牌号" size="small">
        <el-input v-model="formSearch.licenseNum" placeholder="请输入车牌号"></el-input>
      </el-form-item>
      <el-form-item label="访客姓名" size="small">
        <el-input v-model="formSearch.visitorName" placeholder="请输入访客姓名"></el-input>
      </el-form-item>
      <el-form-item label="访客电话" size="small">
        <el-input v-model="formSearch.visitorTel" placeholder="请输入访客电话"></el-input>
      </el-form-item>
      <el-form-item label="是否取消" size="small">
      ...
      <el-form-item label="收费渠道" size="small">
        <el-select v-model="formSearch.payType">
      ...
      <el-form-item label="开始时间" size="small">
        <el-date-picker
          v-model="formSearch.bookArriveTimeStart"
          type="datetime"
      ...
      <el-form-item label="结束时间" size="small">
        <el-date-picker
          v-model="formSearch.bookArriveTimeEnd"
          type="datetime"
          value-format="timestamp"
          placeholder="结束时间">
        </el-date-picker>
    ...
    <el-table :data="formData" style="width: 100%" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="visitorTel" label="访客电话"></el-table-column>
      <el-table-column prop="licenseNum" label="车牌号"></el-table-column>
      <el-table-column prop="carparkNo" label="车场标识"></el-table-column>
      <el-table-column prop=""label="是否已取消">
        <template slot-scope="scope">
          {{scope.row.type==1?'已预约':"已取消"}}
        </template>
      </el-table-column>
      <el-table-column prop=""label="是否需交费">
        <template slot-scope="scope">
          {{scope.row.isCharge==1?'是':"否"}}
        </template>
      </el-table-column>
      <el-table-column prop=""label="入场时间" width=150>>
        <template slot-scope="scope" v-if="scope.row.bookArriveTimeStart">
          {{$root.getDateArray(scope.row.bookArriveTimeStart)[9]}}
        </template>
      </el-table-column>
      <el-table-column prop=""label="离场时间" width=150>
        <template slot-scope="scope" v-if="scope.row.bookArriveTimeEnd">
          {{$root.getDateArray(scope.row.bookArriveTimeEnd)[9]}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" width=250>
				<template slot-scope="scope">
					<el-button type="warning" size="small" @click="updateShowBox(scope.row)">取 消</el-button>
				</template>
			</el-table-column>
    </el-table>
      ...
      <div class="cont_box_left">
        <el-form label-position="right" label-width="110px" :model="formPush" :rules="rules"  ref='addList'>
          ...
          <el-form-item label="访客姓名">
            <el-input v-model="formPush.visitorName"></el-input>
          </el-form-item>
          <el-form-item label="访客电话">
            <el-input v-model="formPush.visitorTel"></el-input>
          </el-form-item>
          <el-form-item label="访客车牌号">
            <el-input v-model="formPush.licenseNum"></el-input>
          </el-form-item>
          <el-form-item label="是否需收费">
            <el-radio v-model="formPush.isCharge" :label="0">否</el-radio>
            <el-radio v-model="formPush.isCharge" :label="1">是</el-radio>
          </el-form-item>
          <el-form-item label="开始时间" size="small">
            <el-date-picker
              v-model="formPush.bookArriveTimeStart"
              type="datetime"
              value-format="timestamp"
              placeholder="开始时间">
            </el-date-picker>
          ...
          <el-form-item label="结束时间" size="small">
            <el-date-picker
              v-model="formPush.bookArriveTimeEnd"
              type="datetime"
              value-format="timestamp"
              placeholder="结束时间">
            </el-date-picker>
...
import { visitorBookRecords,delCar,visitorAdd,updatePropertyTypeList,visitorDel,xqSelectList,orgTree } from '../../url/api';
export default {
  data(){
    return{
      checkStrictly:false,
  ...
  methods:{
    getlist(){
      visitorBookRecords(this.formSearch).then((res)=>{
        console.log(res)
      ...
      // visitorBookRecordsFee(this.formSearch).then((res)=>{
      //   console.log(res)
      //   if(res.data.code == 200){
      //     this.moduleInfor = {
      //       name:'parking_keytop_record',
    ...
    addList(addList){//添加树状图node节点
      this.$refs[addList].validate((valid) => {
        if (valid) {
          visitorAdd(this.formPush).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.getlist()
              this.$message({
