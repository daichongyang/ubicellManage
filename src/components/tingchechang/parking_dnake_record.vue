  ...
  <section class="modlude">
    <el-form :inline="true" :model="formSearch" class="form_inline" label-width="100px">
      ...
      <el-form-item label="车牌号" size="small">
        <el-input v-model="formSearch.cph" placeholder="请输入车牌号"></el-input>
      </el-form-item>
      ...
      <el-form-item label="入场时间" size="small">
        <el-date-picker
    ...
    <el-table :data="formData" style="width: 100%" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="sfGateOut"label="出场收费口"></el-table-column>
      <el-table-column prop="sfGate"label="入场收费口"></el-table-column>
      <el-table-column prop=""label="出场时间" width=150>>
        <template slot-scope="scope" v-if="scope.row.outTime">
          {{$root.getDateArray(scope.row.outTime)[9]}}
        </template>
      </el-table-column>
      <el-table-column prop="outPic"label="出场图片">
        <template slot-scope="scope">
          <el-image  style="width:80px;height:80px;" :src="scope.row.outPic" fit="cover"></el-image>
        </template>
      </el-table-column>
      <el-table-column prop="outGateName"label="出场名称"></el-table-column>
      <el-table-column prop=""label="入场时间" width=150>>
        <template slot-scope="scope" v-if="scope.row.inTime">
          {{$root.getDateArray(scope.row.inTime)[9]}}
        </template>
      </el-table-column>
      <el-table-column prop="inPic"label="入场图片">
        <template slot-scope="scope">
          <el-image style="width:80px;height:80px;" :src="scope.row.inPic" fit="cover"></el-image>
        </template>
      </el-table-column>
      <el-table-column prop="inGateName"label="入场名称"></el-table-column>
      <el-table-column prop=""label="收费时间" width=150>
        <template slot-scope="scope" v-if="scope.row.sfTime">
          {{$root.getDateArray(scope.row.sfTime)[9]}}
        </template>
      </el-table-column>
...
import { carRecords,carRecordsFee,delCar,addPropertyTypeList,updatePropertyTypeList,addReply,xqSelectList,orgTree } from '../../url/api';
export default {
  data(){
    return{
      checkStrictly:false,
  ...
  methods:{
    getlist(){
      carRecords(this.formSearch).then((res)=>{
        console.log(res)
      ...
      // carRecordsFee(this.formSearch).then((res)=>{
      //   console.log(res)
      //   if(res.data.code == 200){
      //     this.moduleInfor = {
