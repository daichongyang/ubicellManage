  ...
  <section class="modlude">
    ...
    <el-table :data="formData" style="width: 100%" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" fixed="right" width=400>
				<template slot-scope="scope">
					<el-button  size="small" @click="updatecsinfoList(scope.row)">{{scope.row.isVerify==1?"重新审核":"审核通过"}}</el-button>
...
import {getgenreid,deleteComment,getlishopinfo,updatecsinfoList,csinfoList,xqSelectList,orgTree,addrecomm } from '../../url/api';
export default {
  data(){
    return{
      headers:{
  ...
  methods:{
    ...
    updatecsinfoList(item){// 修改审核状态
      let params = {
        id:item.id,
        isVerify:item.isVerify==1?2:1
      }
      updatecsinfoList(params).then((res)=>{
          console.log(res)
    ...
    getlist(){
      csinfoList(this.formSearch).then((res)=>{
        console.log(res)
        if(res.data.code == 200){
    ...
    addList(addList){//添加树状图node节点
      this.$refs[addList].validate((valid) => {
        if (valid) {
          addrecomm(this.formPush).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.getlist()
              this.$message({
