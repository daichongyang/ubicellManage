...
import {deletecommentList,commentList,xqSelectList,orgTree } from '../../url/api';
export default {
  data(){
    return{
      checkStrictly:false,
  ...
  methods:{
    getlist(){
      this.formSearch.infoId=this.$route.query.hsId
      commentList(this.formSearch).then((res)=>{
        console.log(res)
    ...
    addList(addList){//添加树状图node节点
      this.$refs[addList].validate((valid) => {
        if (valid) {
          addcommentList(this.formPush).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.getlist()
              this.$message({
    ...
    updateList(formName){//修改
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          updatecommentList(params).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.$message({
                message: '修改成功',
    ...
    deleInfor(ids){//删除
      ...
      this.$confirm('确认删除吗？')
      .then(_ => {
        deletecommentList(arrId).then((res)=>{
          console.log(res)
          if(res.data.code == 200){
            this.$message('删除成功');
            this.getlist()
