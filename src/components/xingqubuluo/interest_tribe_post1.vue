...
import {deletetruecommmDataList,gettruecommmDataList,xqSelectList,orgTree } from '../../url/api';
export default {
  data(){
    return{
      checkStrictly:false,
  ...
  methods:{
    getlist(){
      ...
      gettruecommmDataList(this.formSearch).then((res)=>{
        console.log(res)
    ...
    deleInfor(ids){//删除
      ...
      this.$confirm('确认删除吗？')
      .then(_ => {
        deletetruecommmDataList(arrId).then((res)=>{
          console.log(res)
          if(res.data.code == 200){
            this.$message('删除成功');
            this.getlist()
